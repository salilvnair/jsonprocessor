	...
	public List<ValidationMessage> validate(Object currentInstance, String path,
		...
		if(minLength!=-1 || maxLength!=-1 || length!=-1) {
			...
			if(minLengthViolated) {
				errors = prepareFieldViolationMessage(ValidatorType.MINLENGTH,field,errors,path,"min length error");
			}
			if(maxLengthViolated) {
				errors = prepareFieldViolationMessage(ValidatorType.MAXLENGTH,field,errors,path,"max length error");
			}
			if(lengthViolated) {
				errors = prepareFieldViolationMessage(ValidatorType.LENGTH,field,errors,path,"length error");
			}
		}
		return errors;
	}
